...
Completed backlog items, split out of [TODO.md](./TODO.md) to keep the active
list focused. Section headings mirror TODO.md; each entry keeps its full
description. See TODO.md for work still outstanding.
...
- [x] Either split this file [TODO.md](./TODO.md) into "todo" and "done" as separate files *or* consider using GitHub issues to track items still to do.
      — split into two files: `TODO.md` keeps the still-to-do (`[ ]`) items,
      `DONE.md` collects the completed (`[x]`) ones. Each item's full multi-line
      description and any nested sub-items travel with it; section headings are
      preserved in whichever file has items under them. Partially-done items (an
      open parent with some ticked children) stay in `TODO.md` with the parent.
